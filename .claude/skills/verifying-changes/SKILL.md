...
- **Silent migration failures**: the app boots even if a migration fails (warn-only by default). When verifying schema changes, set `PF_DB_STRICT_MIGRATE=1`.
...
| DB schema / migrations | Ephemeral-DB test with `PF_DB_STRICT_MIGRATE=1` (see changing-db-schema skill) |
